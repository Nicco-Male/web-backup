...
`);
  ...
  it("decodes channel_url when channels list is not present", () => {
    const source = createConfigBackupYaml({
      channels: new Map([
        [
          0,
          create(Protobuf.Channel.ChannelSchema, {
      ...
      config: create(Protobuf.LocalOnly.LocalConfigSchema, {
        device: { role: Protobuf.Config.Config_DeviceConfig_Role.CLIENT },
      }),
    ...
    const parsed = parseConfigBackupYaml(source);

    expect(parsed.errors).toEqual([]);
    expect(parsed.backup?.channels).toHaveLength(2);
    expect(parsed.backup?.channels[0]?.settings?.name).toBe("Primary");
    expect(parsed.backup?.channels[1]?.settings?.name).toBe("Secondary");
  });

  it("parses owner and canned messages fields from cli yaml", () => {
    const yaml = createConfigBackupYaml({
      channels: new Map([
        [
          0,
      ...
      owner: "Node Long",
      ownerShort: "NL",
      cannedMessages: "Hi|Bye|Yes",
    });

    const parsed = parseConfigBackupYaml(yaml);

    expect(parsed.errors).toEqual([]);
    expect(parsed.backup?.owner).toBe("Node Long");
    expect(parsed.backup?.ownerShort).toBe("NL");
    expect(parsed.backup?.cannedMessages).toBe("Hi|Bye|Yes");
  });
});
